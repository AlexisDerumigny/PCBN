...
- File `Graph_B_sets.R`
  ...
  **TODO**: add better visualization of interfering v-structures, as was done
  in `plot_active_cycles()` for the active cycles.
...
- File `PCBN-distances.R`
   ...
   **TODO**: improve and clean this. What do we need? What do we want from this?
...
- File `Hill_climbing.R`
   ...
   + `allowed_operations_fromDAG`, `operation_do` and `operation_undo` work.
...
- File `Random_PCBN.R`
  ...
  **TODO**: fix, improve and clean this.
  What do we need? What do we want from this?
